...
def main(_):
    ...
    if FLAGS.visualize_reconstruct:
        ...
        #saved_step = ae.load()
        saved_step = ae2input.load_fixedNum(14750)
        assert saved_step > 1, "A trained model is needed to encode the data!"
        
        pathForSave='VisualImgsResultsOriRec'
        try:
        ...
        for batch_num in range(int(sampleNum/FLAGS.batch_size)):
            ...
            ae2input.visulize_rec_Origin(pathForSave,img_batch1,img_batch2, batch_num)
            if batch_num < 5 or batch_num % 10 == 0:
                print(("Batch number {0}".format(batch_num)))
           
        print("Swapped images saved to Folder: VisualImgsResultsOriRec")
