...
# upaired
SVHNArray1=np.empty((Num,32,32,3))
SVHNArray2=np.empty((Num,32,32,3))
# paired
...
for imgFolderName in range(1,classes+1,1):
    for k in range(Single_Num):
        ...
        SVHNArray1[(imgFolderName-1)*Single_Num+k] = temp_arr
        SVHNArray2[(imgFolderName - 1) * Single_Num + k] = temp_arr
np.random.shuffle(SVHNArray1) # shuffle
np.random.shuffle(SVHNArray2) # shuffle
...
for i  in range(processNumber):
    ...
    bg=SVHNArray1[BgRndomIndexAux1[i]]
    bg=bg[0:7,0:7,:]
    ...
    bg = SVHNArray1[BgRndomIndexAux2[i]]
    bg = bg[0:7, 0:7, :]
    bg_img = Image.fromarray(np.uint8(bg))
    bg_img = bg_img.resize((width, height))
    bg2 = np.array(bg_img)
...
if not os.path.exists(save_dir):
    os.mkdir(save_dir)
# paired
np.savez(save_dir+'SVHN10WithBg_aux1_GT1_255oneguided_N'+str(processNumber)+'x'+str(width)+'x'+str(height)+'x'+str(ch)+'_train_forDSD.npz',images=Aux1_SVHN2Array,masks=Aux1_SVHN2Array_GT)
np.savez(save_dir+'SVHN10WithBg_aux2_GT2_255oneguided_N'+str(processNumber)+'x'+str(width)+'x'+str(height)+'x'+str(ch)+'_train_forDSD.npz',images=Aux2_SVHN2Array,masks=Aux2_SVHN2Array_GT)
# unpaired
np.savez(save_dir+'SVHN10WithBg_img3_255oneguided_N'+str(processNumber)+'x'+str(width)+'x'+str(height)+'x'+str(ch)+'_train_forDSD.npz',images=SVHNArray1,masks=masksForAux1Array)
np.savez(save_dir+'SVHN10WithBg_img4_255oneguided_N'+str(processNumber)+'x'+str(width)+'x'+str(height)+'x'+str(ch)+'_train_forDSD.npz',images=SVHNArray2,masks=Aux1_SVHN2Array_label)
