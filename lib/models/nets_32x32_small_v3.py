...
def resnet_encoder(name, inputs, n_channels, latent_dim, is_training, mode=None, nonlinearity=tf.nn.relu):
    ...
    output = ResidualBlock(name + '.Res4', 8*DIM, 16*DIM, 3, output, is_training, mode, resample='down')    
    output = tf.reshape(output, [-1, 2*2*16*DIM])
    output = Linear(name + '.Output', 2*2*16*DIM, latent_dim, output)
    
    return output

def resnet_decoder(name, z, n_channels, is_training, mode=None, nonlinearity=tf.nn.relu):
    output = Linear(name + '.Input', z.get_shape().as_list()[1], 2*2*16*DIM, z)
    output = Normalize(name + '.BN0', [0], output, is_training, mode)
    output = nonlinearity(output)
    output = tf.reshape(output, [-1, 16*DIM, 2, 2])

    output = ResidualBlock(name + '.Res1', 16*DIM, 8*DIM, 3, output, is_training, mode, resample='up', norm_inputs=True)
