...
num,classes=label_test.shape
label_test_=np.zeros(shape=(num))
for i in range(num):
    for j in range(classes):
        if label_test[i,j]==1:
            label_test_[i]=j
num,classes=pre.shape
pre_=np.zeros(shape=(num))
for i in range(num):
    for j in range(classes):
        if pre[i,j]==1:
            pre_[i]=j

recall_overall=metrics.recall_score(label_test_,pre_,average='micro')
recall_perclass=metrics.recall_score(label_test_,pre_,average='macro')
precision_overall=metrics.precision_score(label_test_,pre_,average='micro')
precision_perclass=metrics.precision_score(label_test_,pre_,average='macro')

print("C-R: {}".format(recall_perclass))
print("C-P: {}".format(precision_perclass))
